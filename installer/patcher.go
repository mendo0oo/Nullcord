...
func init() {
	if dir := os.Getenv("NULLCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using NULLCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../NullCordData")
		BaseDir = path.Join(dir, "..", "NullCordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("NullCord").UserConfig()
	}
