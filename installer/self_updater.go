...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/mendo0oo/Nullcord/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "NullCordInstallerCli.exe", "NullCordInstaller.exe")
		return BaseUrl + filename
	case "darwin":
		return BaseUrl + "NullCordInstaller.MacOS.zip"
	case "linux":
		return BaseUrl + "NullCordInstallerCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "NullCordInstallerUpdate")
	if err != nil {
