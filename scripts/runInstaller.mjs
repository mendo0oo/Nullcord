...
const BASE_URL = "https://github.com/mendo0oo/Nullcord/releases/latest/download/";
const INSTALLER_PATH_DARWIN = "NullCordInstaller.app/Contents/MacOS/NullCordInstaller";
...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "NullCordInstallerCli.exe";
        case "darwin":
            return "NullCordInstaller.MacOS.zip";
        case "linux":
            return "NullCordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile = process.platform === "darwin"
        ? join(FILE_DIR, "NullCordInstaller")
        : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "NullCord (https://github.com/mendo0oo/Nullcord)",
            "If-None-Match": etag
        }
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(`sudo spctl --add '${outputFile}' --label "NullCord Installer"`);
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...process.env,
            NULLCORD_USER_DATA_DIR: BASE_DIR,
            NULLCORD_DEV_INSTALL: "1"
        }
    });
} catch {
